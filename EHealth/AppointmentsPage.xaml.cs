...
namespace EHealthApp
{
    public partial class AppointmentsPage : ContentPage
    {
        ...
        private async void OnDeleteAppointmentClicked(object sender, EventArgs e)
        {
            ...
            {
                bool confirm = await DisplayAlert("Confirmare", $"Ștergi programarea '{appt.Title}'?", "Da", "Nu");
                if (!confirm)
                    return;

                var notificationService = DependencyService.Get<ILocalNotificationService>();
                notificationService?.CancelNotification(appt.AppointmentDate, appt.Title);

                await _database.DeleteAppointmentAsync(appt);
                _programariAfisate.Remove(appt);

                await DisplayAlert("Succes", "Programare ștearsă!", "OK");
            }
        }
    }
}
